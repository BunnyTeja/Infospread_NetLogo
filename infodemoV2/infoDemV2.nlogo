...
if not file-exists? "agents_with_idsV2.csv" [
user-message "No file 'basic-agents-input' exists."
stop
]

file-open "agents_with_idsV2.csv" ; open the file with the turtle data
...
;
 ...
 set county item 4 data
 ;set latitude item 4 data
 set Municipality item 5 data
 set Coordinates item 6 data
 ...
 set TriadStackID word "TS-" item 1 data
  ...
  repeat count-agents [ ; repeating hundred times for each row i bcz we have hundred columns
    ...
    if value = 1[
     ask turtle i [
          ...
          if out-links?
          [create-link-with turtle j]
        ;ask links [set color red + 2]  ; This draws a line between the agents it's just for representation
        set k k + 1
...
if not file-exists? "IPs-inputV2.csv" [
user-message "No file 'IPs-inputV2.csv' exists."
stop
]

file-open "IPs-inputV2.csv" ; open the file with the turtle data
  ...
  if i = 0[
   ...
   foreach sort turtles [ t ->
    ask t [if agent-type = "basic-agents" or agent-type = "information-diss-agents"[
        set sub-list [ (list agent-type agent-id TriadStackID received-IP-list sent-IP-list country county Municipality Coordinates Gender Age Language Nationality PoliticalSpectrum SocioeconomicStatus EU NATODonovia InformationDisseminationAgents key_issue  i date-and-time)] of t
...
ifelse random 100 < 50 [
   ...
   foreach sort turtles [ t ->
    ...
    set j j + 1
     ...
     csv:to-file "Information-Actions.csv" Information_Action_list
  set i i + 1
]
...
;                          ]
 ...
 csv:to-file "Identity-Actions.csv" identity-list
 csv:to-file "Track-Triads.csv" Triad-list
